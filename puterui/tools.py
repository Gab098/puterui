...
TOOL_DEFINITIONS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "function": {
            "name": "scan_ports",
            "description": (
                "Scan TCP ports on a remote host and report open/closed states. "
                "For legal security testing on authorized targets only."
            ),
            "parameters": {
                "type": "object",
                "required": ["host"],
                "properties": {
                    "host": {
                        "type": "string",
                        "description": "Target hostname or public IP address.",
                    },
                    "ports": {
                        "type": "string",
                        "description": (
                    ...
                    "timeout": {
                        "type": "number",
                        "description": "Socket timeout per port in seconds (default 0.5).",
                    },
                },
            },
        },
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Generate a quick codebase map with language/file counts and key files. "
                "Useful before coding/refactoring."
            ),
            "parameters": {
                "type": "object",
                "required": ["path"],
                "properties": {
                    "path": {
                        "type": "string",
                        "description": "Project subdirectory to analyze ('.' for whole project).",
                    },
                    "max_files": {
                        "type": "integer",
...
def _parse_ports_spec(ports_spec: str | None) -> tuple[list[int], str | None]:
    """Parse a comma-separated list of ports and ranges into a sorted unique list."""
    if not ports_spec:
        return [21, 22, 25, 53, 80, 110, 143, 443, 445, 3306, 5432, 6379, 8080, 8443], None
    ...
    for part in ports_spec.split(","):
        ...
        if "-" in token:
            ...
            if not (start_str.isdigit() and end_str.isdigit()):
                return [], f"Error: invalid port range '{token}'."
            start, end = int(start_str), int(end_str)
            if start > end:
                return [], f"Error: invalid port range '{token}' (start>end)."
            if start < 1 or end > 65535:
                return [], f"Error: port range '{token}' is outside 1-65535."
            if end - start > 2000:
                return [], "Error: port ranges larger than 2000 are not allowed."
            ports.update(range(start, end + 1))
        else:
            if not token.isdigit():
                return [], f"Error: invalid port '{token}'."
            port = int(token)
            if port < 1 or port > 65535:
                return [], f"Error: invalid port '{token}' (must be 1-65535)."
            ports.add(port)

    if not ports:
        return [], "Error: no valid ports provided."
    if len(ports) > 256:
        return [], "Error: too many ports requested (max 256 per scan)."
    return sorted(ports), None


async def tool_scan_ports(args: dict[str, Any]) -> str:
    """Scan remote TCP ports with safety restrictions against local/private targets."""
    host = str(args.get("host", "")).strip()
    if not host:
        return "Error: host is required."

    safety_error = _validate_network_url(f"http://{host}")
    if safety_error:
        return safety_error

    ports, parse_error = _parse_ports_spec(args.get("ports"))
    if parse_error:
        return parse_error
    ...
    async def _scan_one(port: int) -> tuple[int, bool]:
        ...
        is_open = await asyncio.to_thread(_probe)
        return port, is_open

    results = await asyncio.gather(*[_scan_one(port) for port in ports])
    open_ports = [port for port, is_open in results if is_open]
    closed_count = len(ports) - len(open_ports)

    lines = [
        f"Host: {host} ({target_ip})",
        f"Scanned ports: {len(ports)}",
        f"Open ports: {len(open_ports)}",
        f"Closed/filtered ports: {closed_count}",
    ]

    if open_ports:
        top = ", ".join(str(port) for port in open_ports[:80])
        lines.append(f"Open list: {top}")
        if len(open_ports) > 80:
            lines.append("... (open port list truncated)")
...
async def tool_codebase_outline(args: dict[str, Any], project_dir: Path) -> str:
    """Create a high-level codebase outline for planning coding tasks."""
    base = _resolve_path(args.get("path", "."), project_dir)
    if not _is_safe_path(base, project_dir):
        return f"Error: path '{args.get('path', '.')}' is outside the project directory."
    ...
    max_files = min(max(max_files, 50), 20000)
    ...
    for file in sorted(base.rglob("*")):
        ...
        if not file.is_file():
            continue
        rel_from_base = file.relative_to(base)
    ...
    for file in files:
        language = LANGUAGE_BY_SUFFIX.get(file.suffix.lower(), "Other")
        lang_counts[language] = lang_counts.get(language, 0) + 1

    top_langs = sorted(lang_counts.items(), key=lambda x: (-x[1], x[0]))[:8]
    ...
    for pattern in key_file_patterns:
        for file in files:
            if file.name == pattern:
                key_files.append(_to_display_path(file.relative_to(project_dir)))

    sample_files = [_to_display_path(file.relative_to(project_dir)) for file in files[:25]]

    lines = [
        f"Base path: {_to_display_path(base.relative_to(project_dir))}",
        f"Files scanned: {len(files)}",
        "Top languages:",
    ]
    lines.extend([f"- {name}: {count}" for name, count in top_langs])

    if key_files:
        lines.append("Key files:")
        lines.extend([f"- {path}" for path in key_files[:20]])

    lines.append("Sample files:")
    lines.extend([f"- {path}" for path in sample_files])
...
FILE_TOOL_HANDLERS = {
    ...
    "codebase_outline": tool_codebase_outline,
